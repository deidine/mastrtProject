...
export default function PopUpMenu({
  ...
  isSideOpen: (value: boolean,index:number) => void;
  ...
  const toggleSidebar = ( ) => {
    ...
    isSideOpen(!isSideBarOpen,index);
  };

  return (
    <div
      className="absolute -top-24 right-0 z-50 inline-flex items-center justify-center translate-y-[100%] my-auto w-40 bg-white border border-gray-200 rounded-lg shadow-lg"
      ref={menuRef}
    >
      <div
        role="separator"
      ...
      <ul className="flex flex-col p-1 space-y-2 rounded-lg px-1 py-1">
        ...
        >
          {" "}
          {isSideBarOpen ? "Close" : "Open"} Side Bar
        </li>
      </ul>
    </div>
  );
